...
// Sample outputs generated from this workflow, bundled with the site
const exampleVideos = [
  { title: 'POV Concept', src: '/videos/pov-concept.mp4' },
  { title: 'Dramatic Fast-Paced Battle', src: '/videos/dramatic-fast-paced.mp4' },
  { title: 'Generated Video Demo', src: '/videos/generate-the-video.mp4' },
];
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans p-4 md:p-8 flex justify-center">
      <div className="max-w-3xl w-full">
        ...
        <header className="mb-8 text-center md:text-left">
          <a
            href="#"
            className="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-purple-400 transition-colors mb-4"
          >
            <Icons.ChevronLeft /> Back to the blueprint overview
          </a>
        ...
        <section className="mt-10">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <Icons.Video /> Example Outputs
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {exampleVideos.map((video) => (
              <div key={video.src} className="bg-slate-900/80 border border-slate-700/80 rounded-xl overflow-hidden shadow-lg">
                <video
                  src={video.src}
                  controls
                  preload="metadata"
                ...
                />
                <p className="p-3 text-sm text-slate-300 font-medium">{video.title}</p>
              </div>
            ))}
          </div>
        </section>
